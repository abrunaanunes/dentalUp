                ...
                <?php 
                  if(!empty($data)) {
                    foreach($data as $message) {
                      echo '<span style="font-size: 14px; font-weight: 300; line-height: 30px; color: red;">' . $message . '</span>';
                    }
                  }
                ?>

                <form class="form"  action="<?php echo 'http://' . $_SERVER['HTTP_HOST']. '/' . 'login'; ?>" method="post"  action="<?php echo 'http://' . $_SERVER['HTTP_HOST']. '/' . 'login'; ?>" method="post">
